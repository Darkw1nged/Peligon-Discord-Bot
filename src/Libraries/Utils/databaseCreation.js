module.exports.initializeDatabase = function (client, databaseConnection) {
    try {
        ...
        databaseConnection.query(`CREATE TABLE IF NOT EXISTS channels (guild_id VARCHAR(255) NOT NULL, welcome VARCHAR(255), log VARCHAR(255), rankup VARCHAR(255), category_games VARCHAR(255), FOREIGN KEY (guild_id) REFERENCES guilds(guild_id))`);
        ...
        databaseConnection.query(`CREATE TABLE IF NOT EXISTS leaderboard (guild_id VARCHAR(255) NOT NULL, user_id VARCHAR(255) NOT NULL, experience INT DEFAULT 0, coins INT DEFAULT 0, bank INT DEFAULT 0, wordle INT DEFAULT 0, wordle_best INT DEFAULT 0, word_search INT DEFAULT 0, word_search_best INT DEFAULT 0, FOREIGN KEY (guild_id) REFERENCES guilds(guild_id))`);
        ...
        databaseConnection.query(`CREATE TABLE IF NOT EXISTS word_search (guild_id VARCHAR(255) NOT NULL, user_id VARCHAR(255) NOT NULL, channel_id VARCHAR(255) NOT NULL, difficulty VARCHAR(255) NOT NULL, board TEXT NOT NULL, words TEXT NOT NULL, FOREIGN KEY (guild_id) REFERENCES guilds(guild_id))`);
        databaseConnection.query(`CREATE TABLE IF NOT EXISTS word_search_profile (guild_id VARCHAR(255) NOT NULL, user_id VARCHAR(255) NOT NULL, games_won INT DEFAULT 0, forfeited INT DEFAULT 0, games_won_streak INT DEFAULT 0, games_won_best_streak INT DEFAULT 0, FOREIGN KEY (guild_id) REFERENCES guilds(guild_id))`);
    ...
    client.guilds.cache.forEach(async (guild) => {
        const databaseUtils = require('./UDatabase');
        databaseUtils.isGuildInDatabase(guild).then(async (result) => {
            if (!result) {
                ...
                databaseConnection.query(`INSERT INTO channels (guild_id, welcome, log, rankup, category_games) VALUES ('${guild.id}', 0, 0, 0, 0)`);                 
